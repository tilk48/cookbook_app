...
class RecipeDetailProvider extends ChangeNotifier {
  ...
  bool get hasServingAdjustment => _originalServings != null && _originalServings! > 0;
  
  /// Get completion status of a cooking step
  bool isStepCompleted(int stepIndex) => _completedSteps[stepIndex] ?? false;
  
  /// Get count of completed steps
  int get completedStepsCount => _completedSteps.values.where((completed) => completed).length;
  ...
  Future<void> loadRecipe(String slug) async {
    ...
    try {
      ...
      _originalServings = _parseServings(recipe.recipeYield, recipe.recipeServings);
      ...
      print('DEBUG: Recipe has ${ingredients.length} ingredients and ${instructions.length} instructions');
  ...
  Future<void> toggleFavorite() async {
    ...
    try {
      ...
      print('DEBUG: Toggled favorite status for ${_recipe!.name}: $_isFavorite');
    } catch (e) {
      // Revert on error
      _isFavorite = !_isFavorite;
      notifyListeners();
  ...
  void setTargetServings(int targetServings) {
    if (targetServings > 0 && targetServings <= 50 && _originalServings != null && _originalServings! > 0) {
  ...
  String getFormattedIngredient(RecipeIngredient ingredient, {bool scaled = false}) {
    final parts = <String>[];
    
    try {
      // Add quantity and unit
      if (ingredient.quantity != null && !(ingredient.disable_amount ?? false)) {
        ...
        if (ingredient.unit?.name.isNotEmpty == true) {
          ...
          final unitName = (ingredient.unit!.useAbbreviation == true && ingredient.unit!.abbreviation?.isNotEmpty == true)
  ...
  String _formatQuantity(double quantity) {
    ...
    if (quantity == quantity.floor()) {
      ...
      return quantity.toStringAsFixed(quantity.truncateToDouble() == quantity ? 0 : 2);
    }
  }
}